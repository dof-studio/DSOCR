        ...
        if sam_model is not None and (input_ids.shape[1] != 1 or self.training) and torch.sum(images[0][1]).item() != 0:
            ...
            for image, crop_shape in zip(images, images_spatial_crop):
                ...
                if images_in_this_batch:
                    ...
                    # This line has problems. Use new version _Compatible please
...
# Type Unified Model
class DeepseekOCRModel_Compatible(DeepseekV2Model):
    config_class = DeepseekOCRConfig

    def __init__(self, config: DeepseekV2Config):
        super(DeepseekOCRModel_Compatible, self).__init__(config)
    ...
    def forward(
        ...
        return super(DeepseekOCRModel_Compatible, self).forward(
            input_ids=None, attention_mask=attention_mask, past_key_values=past_key_values,
            inputs_embeds=inputs_embeds, use_cache=use_cache, position_ids = position_ids,
            output_attentions=output_attentions, output_hidden_states=output_hidden_states,
            return_dict=return_dict
...
# API Model (Call the Unified Model)
class DeepseekOCRForCausalLM(DeepseekV2ForCausalLM):
    ...
    def __init__(self, config):
        ...
        # Changed here
        self.model = DeepseekOCRModel_Compatible(config)
    ...
    def forward(
        ...
        # Dynamically detect the runtime container type
        runtime_dtype = self.model.dtype
        # This is to ensure CPU Inference
    ...
    # Original Inference API (Native, deprecated)
